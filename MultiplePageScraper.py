...
def main():
	# Main link
	# Just change the range's second parameter to modify the number of page that will be scraped. 
	totalPage = [str(i) for i in range(1,3)]
	mainURL="https://opinion.inquirer.net/tag/rodrigo-duterte"
	
	for pageNum in totalPage:
		pageUrl = mainURL + '/page/' + pageNum
		print (pageUrl)
	
		# Main link to Bsoup
		mainBSoup = converToSoup(pageUrl)
